...
export default {
  ...
  methods: {
    ...
    moveOnDrop(dragAndDrop) {
      if (dragAndDrop.status !== this.DND_STATUS.INTERNAL) {
        return
      }

      let dragNode = dragAndDrop.dragNode
      let dropNode = dragAndDrop.overNode
    ...
    async dropEvent(event) {
      ...
      const copyDragAndDrop = this.getShallowCopyOfDragAndDrop()

      if (typeof(this.fnBeforeDrop) === 'function' && await this.fnBeforeDrop(this.dragAndDrop) === false) {
        return
      }

      if (copyDragAndDrop.status === this.DND_STATUS.INTERNAL && this.dropToMove === true) {
        this.moveOnDrop(copyDragAndDrop)
      } else {
        this.$emit('drop', copyDragAndDrop, event)
        if (copyDragAndDrop.status === this.DND_STATUS.INTO) {
          this.dragLeave(this.copyDragAndDrop.overNode)
        }
      }
    },
